...
# Generador de mapa random. Con funcionalidad para hacer zoom.
...
class GameWindow < Gosu::Window
  ...
  def button_down(id)
    ...
    @map = generate_map if id == Gosu::KB_SPACE
  end


  def update
    adjust_zoom(0.005) if button_down?(Gosu::KB_DOWN)
    adjust_zoom(-0.005) if button_down?(Gosu::KB_UP)
    set_caption
  end
  ...
  def draw
    tiles_x.times do |x|
      tiles_y.times do |y|
        @map[x][y].draw(x * TILE_SIZE * @zoom, y * TILE_SIZE * @zoom, 0, @zoom, @zoom)
      end
    end
  end
  ...
  def set_caption
    self.caption = 'Perlin Noise. ' << "Zoom: #{'%.2f' % @zoom}. " << 'Use Up/Down to zoom. Space to regenerate.'
  end
  ...
  def load_tiles
    tiles = Gosu::Image.load_tiles(media_path('ground.png'), 128, 128, tileable: true)
    @sand = tiles[0]
    @grass = tiles[8]
    @water = Gosu::Image.new(media_path('water.png'), tileable: true)
  end
